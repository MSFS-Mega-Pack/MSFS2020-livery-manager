...
/**
 * Gets the latest feed from the API
 *
 ...
 */
export default async function GetActiveFeed(CurrentLocale) {
  let feed;
  try {
    feed = (await FetchAndParseJsonManifest(`${ActiveApiEndpoint}/${Constants.api.get.feed}`)).data;
  } catch {
    return CurrentLocale.translate('helpers.get_active_feed.fetch_fail');
  }
    ...
    feed.feed.map(async article => {
      ...
      if (article.article) {
        try {
          ...
          text = CurrentLocale.translate('helpers.get_active_feed.fetch_fail_article_text');
        }
      }
